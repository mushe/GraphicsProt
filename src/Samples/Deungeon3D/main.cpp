...
void MovePlayer(std::vector<std::vector<int>> &dungeonData, glm::ivec2 direction)
{
    if(direction.x == -1)
    {
        if(playerDir.x == 1) playerDir = glm::ivec2(0, -1);
        else if(playerDir.y == -1) playerDir = glm::ivec2(-1, 0);
        else if(playerDir.x == -1) playerDir = glm::ivec2(0, 1);
        else if(playerDir.y == 1) playerDir = glm::ivec2(1, 0);
    }
    ...
    auto nextPos = playerPos - playerDir * direction.y;
...
int main()
{
    ...
    // create mesh
    auto mesh = Mesh::FromOBJ("../Models/Cube/Cube.obj", 4);
    //auto mesh = Mesh::FromOBJ("../Models/Grass/Grass.obj", 4);
    auto tex = Texture::FromPNG("../Textures/DungeonFloor.png");
    const int instanceCount = gridNum * gridNum;
    ...
    {
        instancingBuffer[i].pos = Vec3(0);
        instancingBuffer[i].type = Random::Range(0, 1);
    }
    ...
    while (engine->Running())
    {
        ...
        if(Input::KeyDownNow(KeyCode::M)) showMap = !showMap;

        OrbitalControl::SetPhi(atan2(playerDir.x, -playerDir.y) + 0.57f);
        OrbitalControl::Update(camera, 0.0f, 0.0f);
        ...
        // draw 3d meshes
        int index = 0;
        Vec3 cameraOffs = Vec3(playerPos.x, yOffs, playerPos.y);
        for(int i = 0; i < gridNum; i++)
        {
            for(int j = 0; j < gridNum; j++)
            {
                Vec3 posOffs = Vec3(0);
                if(dungeon[i][j] == Grid::EMPTY) posOffs = Vec3(0.0f, 1.0f, 0.0f);

                instancingBuffer[index].pos = Vec3((float)j, 0, (float)i) - cameraOffs + posOffs;
                instancingBuffer[index].type = dungeon[i][j];
                index++;
            }
        }
        ...
        if(showMap) DisplayDungeonData(dungeon);
