...
layout(binding = 2)
buffer InstancingShaderStrageBufferObject
{
...
layout(location = 7) out int outType;


void main()
{
    int id = gl_InstanceIndex;
    vec3 position = inPosition * ubo.scale + instancingSSBO.params[id].position;

    outColor = vec4(0);
    int type = instancingSSBO.params[id].type;
    if(type == 0) outColor = vec4(0.1, 0.1, 0.1, 1.0);
    else if(type == 1) outColor = vec4(1.0, 1.0, 1.0, 1.0);
    else if(type == 2) outColor = vec4(1.0, 0.0, 0.0, 1.0);
    else if(type == 3) outColor = vec4(0.0, 0.0, 1.0, 1.0);
