...
float rand(vec2 p)
{
    return (fract(sin( dot(p, vec2(127.1,311.7))) * 43758.5453123) - 0.5) * 2.0;
}
...
void main()
{
    ...
    float scaleLeap = step(PerlinNoise3D(vec3(0.0, 0.0, commonUBO.time * ubo.leapSpeed)), ubo.leapThreshold) * ubo.leapScale;
    float scale = ubo.scale + scaleLeap;

    texCoord += vec2(rand(vec2(0.0, texCoord.y + commonUBO.time)), 0.0) * scale * 0.001;
    vec4 tex = texture(inSampler, vec2(1.0 - texCoord.x, texCoord.y));
    outColor = tex;
}