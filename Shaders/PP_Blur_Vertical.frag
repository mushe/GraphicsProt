...
void main()
{
    ...
    vec2 uv_rr = inTexCoord + vec2(0, pixelOffset.y * 2.0 * customUBO.scale);
    vec2 uv_r = inTexCoord + vec2(0, pixelOffset.y * customUBO.scale);
    vec2 uv_l = inTexCoord + vec2(0, -pixelOffset.y * customUBO.scale);
    vec2 uv_ll = inTexCoord + vec2(0, -pixelOffset.y * 2.0 * customUBO.scale);
