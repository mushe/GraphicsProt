...
compiler_path = 'C:\\VulkanSDK\\1.3.268.0\\Bin\\glslc.exe'
if os_name == 'Darwin':
...
frag_sample_shaders = glob.glob(f"src/**/*.frag", recursive=True)
vert_sample_shaders = glob.glob(f"src/**/*.vert", recursive=True)
comp_sample_shaders = glob.glob(f"src/**/*.comp", recursive=True)
